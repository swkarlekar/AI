...
class Node(object):
	...
	def randomChild(self, num): #choose a random letter from children of current node #choice(List()) from random library
	...
	def searchForNextLetter(self, stng): #recursive #runs through tree to final character in string and then calls random child
		if len(stng) == 0:
			return self.randomChild(len(self.children))
		self = self.children[stng[0]]
		stng = stng[1:]
